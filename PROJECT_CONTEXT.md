...
**Phase 15 complete — admin customers, coupons, reviews moderation, and enquiries inbox.** Four admin screens, all server-wrapped (`/admin` sandboxed layout, `AdminAuthContext` gate, client-side in-memory stores with `TODO(Phase N)` markers for the future Mongoose swap), backed by the existing dummy data in `/src/lib/data` (customers, coupons, reviews, enquiries) plus small type/dummy extensions listed below:

- **`/admin/customers`** (`CustomersManager`, `CustomerDetail`) — list view computing, per customer, name, mobile, email, total orders, total spend, last order date, and registration date (derived by joining `customers` against the `orders` dummy dataset); each row links to `/admin/customers/[id]` detail showing that customer's orders, enquiries (unified from wholesale/wedding/contact), and submitted reviews.
- **`/admin/coupons`** (`CouponsManager`, `CouponForm` new + edit) — list plus create/edit form (code, percentage or fixed discount, minimum order, **maximum discount cap**, **usage limit**, **per-user limit**, start/expiry date) with an enable/disable toggle. Extended the `Coupon` interface and dummy data with the three new fields.
- **`/admin/reviews`** (`ReviewsManager`) — moderation queue with Approve / Feature / Reject / Reset / Delete actions; added `status: ReviewStatus` (`"pending" | "approved" | "rejected" | "featured"`) to the `Review` type and dummy data; only `"approved"` reviews surface on the public `/reviews` page and product-review surfaces (`/flowers/[slug]`).
- **`/admin/enquiries`** (`EnquiriesManager`) — unified inbox merging `wholesaleEnquiries`, `weddingEnquiries`, and `contactEnquiries` with type filter tabs; status pipeline "New → Contacted → In Discussion → Converted → Closed" via per-row status buttons; widened each enquiry type's `status` union (`"inDiscussion"`, `"converted"`, `"closed"`).

Type/dummy-data extensions: `Review.status?: ReviewStatus`; `Coupon.maxDiscountCap?`, `usageLimit?`, `perUserLimit?`; unified `EnquiryStatus = "new" | "contacted" | "inDiscussion" | "converted" | "closed"` across all three enquiry types; all 12 reviews have statuses (8 approved/featured, 4 pending); all 9 enquiries use unified statuses; coupons have the three new fields.

New `Phase15Provider` (`src/components/providers/Phase15Provider.tsx`) seeds all four entity arrays from static data with localStorage persistence via `savePhase15`. Mounted in the admin layout between `OperationsProvider` and children. Fixed Coupon nav role from `[]` to `[AdminRole.ContentManager, AdminRole.OrderManager]` in `src/lib/admin/navigation.ts`.

Verified build/lint: **94 routes**, lint 0 errors (same pre-existing unrelated `connect.ts` warning). `npm run lint` and `npm run build` pass cleanly. Verified headless: the 4 admin routes compile and render; admin shell loads after auth (requires logged-in session at `/admin/login` with `admin@freshflower.zone` / `demo123`); homepage and public pages unaffected.

Awaiting confirmation before Phase 16.

**Phase 16 complete — admin blog CMS, homepage manager, SEO overrides, admin users & roles, notifications inbox, and business settings.** A new Phase 16 store (`SiteContentProvider`, mounted once at the root layout between `WishlistProvider` and `SiteChrome`, key `ff-phase16-v1`) holds all CMS state — blog posts, homepage config, SEO overrides, admin users/passwords, business settings — seeded from `/src/lib/data` and persisted to localStorage. Both the public site and the admin shell consume the same instance, so edits in `/admin/*` appear live on the public pages immediately (and vice-versa render consistently because the seed doubles as both the server render and pre-hydration client state). The hydrate/persist cycle runs through the reducer (`hydrated` flag lives in reducer state, so no effect calls setState — keeping the `react-hooks/set-state-in-effect` rule happy).

- **`/admin/blog`** (`BlogsManager`, `BlogForm`) — grid list with status filter (All / Published / Draft), create/edit form with slug auto-generation from title, rich sections repeater (±), SEO fields (seoTitle, metaDescription, keywords), cover preset swatches, and draft/publish toggle. Public `/blog` reads the live store: drafts stay hidden, new posts appear sorted by `publishedAt` desc. (Server `BlogIndexClient` now consumes the store instead of a `posts` prop; `/blog/[slug]` detail pages still render server-side from seed `blogs` only — a documented preview limitation.)
- **`/admin/homepage`** (`HomepageManager`) — hero editor (eyebrow, title lines + accent line, subtitle, CTA labels/hrefs, video toggle), featured-flower picker (from `useCatalog().products`), offers editor (badge/title/copy/CTA with add/remove), testimonials picker (from `@/lib/data/reviews`, approved/featured only), and a "fresh today" toggle. Public `HomepageClient` renders all of these from the store — offers replaced the previous static gold wholesale CTA, hero testimonials are config-driven, and the product grid reflects `featuredFlowerIds`.
- **`/admin/seo`** (`SeoManager`) — grouped route registry (`ROUTE_REGISTRY` / `ROUTE_GROUPS` in `src/lib/admin/routes.ts`, ~24 routes incl. dynamic `[slug]` patterns) with expandable per-route title/description/keywords editors, save + reset. Applied at runtime by client `SeoMetaInjector` (exact path match first, then `[slug]` pattern match; admin routes and 404s are skipped).
- **`/admin/users`** (`UsersManager`) — list + inline add user (name/email/role/password/active), role select (SuperAdmin/ContentManager/OrderManager), password change, active toggle, delete. Guards: can't deactivate/delete yourself, can't remove the last active superadmin. `AdminAuthContext.login` now reads the live store (`users` + `userPasswords`), so role changes take effect on next sign-in; session key unchanged `ff-admin-session-v1`.
- **`/admin/notifications`** (`NotificationsManager`) — read-only simulated feed (`src/lib/admin/notifications.ts`) DERIVED from `usePhase15().orders`, so status changes on `/admin/orders` show up automatically. Channel tabs (WhatsApp/SMS/Email) with per-channel stats, search, and expandable template bodies.
- **`/admin/settings`** (`SettingsManager`) — full business settings editor (phone, WhatsApp, email, address, porter/delivery notes, Instagram, business hours repeater) with a live preview column (public-style contact card + floating WhatsApp button) and a nullable-`draft` pattern so a hard reload before hydration never clobbers persisted values. Connected to the whole public site: `SiteNav` phone, `SiteFooter` (now `"use client"`) phone/email/address/hours, `ContactInfoCards` on `/contact`, and a new `WhatsAppFloat` button (`src/components/sections/WhatsAppFloat.tsx`) mounted in `SiteChrome` render on every public page. `telHref`/`waMeHref` helpers added to `src/lib/utils.ts`; default WhatsApp `+91 85069 51873` is the single source of truth in `DEFAULT_SETTINGS`.

Type/data extensions: `BlogPostStatus` + optional CMS fields on `BlogPost` (`status`, `seoTitle`, `metaDescription`, `keywords`); `OfferBanner`, `HomepageHeroConfig`, `HomepageConfig`, `BusinessSettings`, `SeoRouteOverride` in `src/lib/types`; new `src/lib/data/homepage.ts` (`DEFAULT_HOMEPAGE`) and `src/lib/data/settings.ts` (`DEFAULT_SETTINGS`); `src/lib/admin/phase16.ts` (store key, seed, merge, `passwordFor`).

Lint/build: `npm run lint` 0 errors (same pre-existing unrelated `connect.ts` unused-disable warning), `npm run build` passes — **105 routes**, the 8 new admin pages (`/admin/blog`, `/admin/blog/new`, `/admin/blog/[id]/edit`, `/admin/homepage`, `/admin/seo`, `/admin/users`, `/admin/notifications`, `/admin/settings`) all present. Also fixed during this phase: `AdminRole` imported as a value (not `import type`), `OrderStatus` enum values used directly in `STAGE_ORDER`, and a leftover `...draft` spread in `SettingsManager`'s remove-row that would have crashed the settings editor.

Verified headless: homepage renders hero from config (eyebrow/title), the gold offers CTA ("A lot of flowers? Let's make it effortless." → `/wholesale`), testimonial cards from `testimonialReviewIds`, and the floating WhatsApp button (`https://wa.me/918506951873`); nav + footer show the settings phone (`tel:+918506951873`) with no hardcoded 89209… remnants; `/contact` shows ContactInfoCards (WhatsApp, phone, hours `6:00 AM – 9:00 PM`, address) and all tel/wa links point at the configured number; 0 console errors, 0 failed requests.
